...
int bullet2_x=-30;
int touches;
NSString *level1=@"NO", *level2=@"NO", *level3=@"NO";
int level=1;
NSString *sound;
NSString *music;
NSString *endOfLevel = @"OFF";
NSString *bombExp=@"OFF";
NSString *cashed=@"OFF";
NSString *angel=@"OFF";
NSString *shot=@"OFF";
NSString *shot2=@"OFF";
...
@synthesize TheAudio, TheAudio2, TheAudio3, TheAudio4, playButton, pauseButton, powerButton, starburst, bomb,bomb2,bomb3,bomb4,bomb5,Timer, gameCenterManager, dude1, explosion, money,money1, money2, money3, cashAudio, cashAudio2, bombAudio, gruntAudio, jumpAudio, hundred, burst, splash, screamAudio, beepAudio, thrustAudio, thrustAudio2, beeps, angelTime, appPlayer, bullet1, bullet2, blood;
...
//	NSLog(@"Game::init");
    ...
    // character will be randomly selected
    switch(arc4random()%7) {
            
        case 0: dude1 = [AtlasSprite spriteWithRect:CGRectMake(577,3,55,65) spriteManager:spriteManager2];break;
        case 1: dude1 = [AtlasSprite spriteWithRect:CGRectMake(435,3,45,79) spriteManager:spriteManager2];break;
        case 2: dude1 = [AtlasSprite spriteWithRect:CGRectMake(480,4,55,55) spriteManager:spriteManager2];break;
        case 3: dude1 = [AtlasSprite spriteWithRect:CGRectMake(636,4,55,55) spriteManager:spriteManager2];break;
        case 4: dude1 = [AtlasSprite spriteWithRect:CGRectMake(56,1,57,55) spriteManager:spriteManager2];break;
        case 5: dude1 = [AtlasSprite spriteWithRect:CGRectMake(105,3,42,74) spriteManager:spriteManager2];break;
        case 6: dude1 = [AtlasSprite spriteWithRect:CGRectMake(160,4,58,70) spriteManager:spriteManager2];break;    
            
	}

    [spriteManager2 addChild:dude1 z:4 tag:kDude1];
    
    // Check positioning for thruster and pause buttons
    ...
    [explosion setPosition:ccp(-10,-50)];
 ...
 // !!!: Timer plus other variables (touches, bombs, angels, bullets)
    timer = 30.00;
    int lowerBound = 75;
    int upperBound = 300;
    touches = 0;
    ...
    int bully = (lowerBound + arc4random() % (upperBound - lowerBound));
    bullet1_y = bully;
    int bully2 = (lowerBound + arc4random() % (upperBound - lowerBound));
    bullet2_y = bully2;
   
    
    bomb = [AtlasSprite spriteWithRect:CGRectMake(368,29,25,46) spriteManager:spriteManager2];
    bomb.position = ccp(lowerBound + arc4random() % (upperBound - lowerBound),1000);
    ...
    bomb2.position = ccp(lowerBound + arc4random() % (upperBound - lowerBound),1050);
    ...
    bomb3.position = ccp(lowerBound + arc4random() % (upperBound - lowerBound),1100);
    ...
    bomb4.position = ccp(lowerBound + arc4random() % (upperBound - lowerBound),1150);
    ...
    bomb5.position = ccp(lowerBound + arc4random() % (upperBound - lowerBound),1200);
    ...
    // Chopstix positioning routine
    int t;
    float delta = 25;
    t = kPlatformsStartTag;
    
   // Creates random positioning for chopstix
    for(t; t < kPlatformsStartTag + kNumPlatforms1; t++) {
        AtlasSprite *platform = (AtlasSprite*)[spriteManager2 getChildByTag:t];
    ...
    // Labels and informative setup
    
    scoreLabel = [[LabelAtlas  labelAtlasWithString:@"" charMapFile:@"casualLetters.png" itemWidth:16 itemHeight:24 startCharMap:'.'] retain];
    [self addChild:scoreLabel z:5];
    [scoreLabel setPosition:ccp(55, 5)];
    
    scoreLabel2 = [[LabelAtlas  labelAtlasWithString:@"" charMapFile:@"casualLetters.png" itemWidth:16 itemHeight:24 startCharMap:'.'] retain];
    [self addChild:scoreLabel2 z:5];
    [scoreLabel2 setPosition:ccp(90, 410)];
    ...
    if ([flip isEqualToString:@"TL"]){
        ...
        touchLabel.position = ccp(275,124);
    }
    ...
    [app.level1Audio play];
  
    
    toc = timer;
    updateLabel.visible = YES;
    [updateLabel setString:@"      LEVEL 1"];
    [updateLabel draw];   
...
//	NSLog(@"initPlatforms");
	...
	while(currentPlatformTag < kPlatformsStartTag + kNumPlatforms1) {
		[self initPlatform];
		currentPlatformTag++;
	}
...
//	NSLog(@"resetPlatforms");
	...
	while(currentPlatformTag < kPlatformsStartTag + kNumPlatforms1) {
		//NSLog(@"kNumPlatforms1: %i",kNumPlatforms1);
        [self resetPlatform];
		currentPlatformTag++;
	}
...
//	NSLog(@"resetBird");
  ...
  //  AtlasSprite *dude1 = (AtlasSprite*)[spriteManager2 getChildByTag:kDude1];
	
	bird_pos.x = 160;
	bird_pos.y = 460;
	bird.position = bird_pos;
    dude1.position = bird_pos;
...
//	NSLog(@"Game::step");
	...
	if(gameSuspended) return;
     ...
     [self performSelector:@selector(dropBullet1) withObject:nil afterDelay:10];
    ...
    CGPoint where = dude1.position;
    where.y = where.y - 2;
    dude1.position = ccp(where.x, where.y);
    tempx = where.x;
    tempy = where.y;
    
	
    CGSize bird_size = dude1.contentSize;
    ...
    float bomb2_max_x = 320-bomb_size.width/2;
	float bomb2_min_x = 0+bomb_size.width/2;
    float bomb3_max_x = 320-bomb_size.width/2;
	float bomb3_min_x = 0+bomb_size.width/2;
...
// !!!: Game over
        // Game Over
        if(dude1.position.y < -bird_size.height/2) {
        ...
        NSString *scoreStr2 = [NSString stringWithFormat:@"SCORE:%d",score - (touches * 25)];
        
        [scoreLabel2 setString:scoreStr2];
        [scoreLabel2 draw];      
        
        if ((score - (touches * 25) >= 1000) && (score - (touches * 25) <= 1100)) {
            
            toc = timer;
            updateLabel.visible = YES;
            [updateLabel setString:@"  LEVEL 2 STATUS - ADDED A LIFE"];
            [updateLabel draw];   
            
            if ((score - (touches * 25) >= 1000) && (score - (touches * 25) <= 1010)){
                AppDelegate *app = (AppDelegate *)[[UIApplication sharedApplication] delegate];
 ...
 // TODO: Time check
    // if the time expires - end ********************* End of Level 1
    if (timer < 0) {
        AppDelegate *app = (AppDelegate *) [[UIApplication sharedApplication] delegate];
        [app.gameLoop stop];
        [app.gameLoop2 stop];
...
#pragma mark Jumps, Drops, Grabs and Deaths
- (void)jump {
    ...
    // gives your dude a slight boost
...
- (void) shot {
    ...
    blood.position = ccp(dude1.position.x,dude1.position.y);
    for (int i = 1;i < 150;i++) {
...
- (void) explode {
    ...
    NSLog(@"Bomb position: %f, %f",bomb.position.x, bomb.position.y);
    NSLog(@"Bomb position: %f, %f",bomb2.position.x, bomb2.position.y);
    NSLog(@"Bomb position: %f, %f",bomb3.position.x, bomb3.position.y);
    ...
    int lowerBound = 75;
    int upperBound = 285;
    bomb.position = ccp(lowerBound + arc4random() % (upperBound - lowerBound),510);
    bomb2.position = ccp(lowerBound + arc4random() % (upperBound - lowerBound),510);
    bomb3.position = ccp(lowerBound + arc4random() % (upperBound - lowerBound),510);
...
- (BOOL)ccTouchBegan:(UITouch *)touch withEvent:(UIEvent *)event {
    ...
    if ([flip isEqualToString:@"TL"]) {
        rect = CGRectMake(280,424,35,45);
	}else if ([flip isEqualToString:@"TR"]){
        rect = CGRectMake(0,424,35,45);       
    }else if ([flip isEqualToString:@"BL"]){
        rect = CGRectMake(280,70,35,45);      
    }else if ([flip isEqualToString:@"BR"]){
        rect = CGRectMake(0,70,35,45);      
    }
