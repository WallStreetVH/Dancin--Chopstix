...
@synthesize TheAudio, TheAudio2, TheAudio3, TheAudio4, playButton, pauseButton, powerButton, starburst, level2Button, bomb,bomb2,bomb3,bomb4,bomb5,Timer, gameCenterManager, explosion, dude3, money,money1, money2, money3, cashAudio, cashAudio2, bombAudio, gruntAudio, hundred, burst, screamAudio, thrustAudio, beepAudio, beeps, splash, angelTime, bullet1, bullet2, blood;
...
- (id)initWithScore:(int)lastScore  {
    ...
    [spriteManager2 addChild:dude3 z:4 tag:kDude3];
    ...
    playButton = [AtlasSprite spriteWithRect:CGRectMake(0,112,72,72) spriteManager:spriteManager2];
    ...
    level2Button = [AtlasSprite spriteWithRect:CGRectMake(306,264,210,40) spriteManager:spriteManager2];
    [spriteManager2 addChild:level2Button z:4 tag:kPlay];
    level2Button.position = ccp(175, 400);
   [level2Button setVisible:NO];
   
    hundred = [AtlasSprite spriteWithRect:CGRectMake(70,225,68,40) spriteManager:spriteManager2];
...
- (void)resetBird {
    ...
    AtlasSprite *dude3 = (AtlasSprite*)[spriteManager2 getChildByTag:kDude3];
...
- (void)step:(ccTime)dt {
  ...
  /*  if ([music isEqualToString:@"ON"]){
   ...
   /* [self performSelector:@selector(dropBomb) withObject:nil afterDelay:1];
    [self performSelector:@selector(dropBomb2) withObject:nil afterDelay:3];
  ...
  //  [self performSelector:@selector(dropBomb5) withObject:nil afterDelay:9];
    ...
    //   [self performSelector:@selector(dropBomb4) withObject:nil afterDelay:15];
    //   [self performSelector:@selector(dropBomb5) withObject:nil afterDelay:16];
    ...
    AtlasSpriteManager *spriteManager2 = (AtlasSpriteManager*)[self getChildByTag:kSpriteManager2];
	dude3 = (AtlasSprite*)[spriteManager2 getChildByTag:kDude3];
    ...
    CGSize bomb5_size = bomb5.contentSize;
    ...
    float bomb4_max_x = 320-bomb4_size.width/2;
	float bomb4_min_x = 0+bomb4_size.width/2;

    float bomb5_max_x = 320-bomb5_size.width/2;
	float bomb5_min_x = 0+bomb5_size.width/2;
	...
	if(bird_vel.y < 0) {
		...
		for(t; t < kPlatformsStartTag + kNumPlatforms3; t++) {
            ...
            CGSize bomb5_size = bomb5.contentSize;
			CGPoint bomb5_pos = bomb5.position;
            ...
            bomb5_max_x = bomb5_pos.x - bomb5_size.width/2 - 1;
			bomb5_min_x = bomb5_pos.x + bomb5_size.width/2 + 1;
			float bomb5_min_y = bomb5_pos.y + (bomb5_size.height+bird_size.height)/2 - kPlatformTopPadding;
            ...
            if(bird_pos.x >= bomb5_max_x &&
               bird_pos.x <= bomb5_min_x &&
               bird_pos.y >= bomb5_y &&
               bird_pos.y <= bomb5_min_y) {
        ...
        if(dude3.position.y < -bird_size.height/2) {
          ...
          //  [TheAudio release];
            ...
            if ([bombExp isEqualToString:@"OFF"])
                lives -= 1;            [splash setPosition:ccp(150,90)];
...
- (void)showHighscores {
   ...
   self.isAccelerometerEnabled = NO;
	...
	changePlayerAlert.title = @"Congratulations!";
	changePlayerAlert.message = @"You have completed the final level alive, your score has been successfully reported to Game Center\n Tap OK to play again.";
	changePlayerAlert.delegate = self;
	[changePlayerAlert addButtonWithTitle:@"OK"];
